...
func UpdateScore(c *gin.Context) {
	...
	for _, player := range req.Players {
		...
		_, err := DB.Redis.ZAdd(ctx, leaderboard_key, redis.Z{
			Member: member,
...
func GetScore(c *gin.Context) {
	...
	for _, player := range req.Players {
		...
		score, err := DB.Redis.ZScore(ctx, leaderboard_key, fmt.Sprint(player.PlayerID)).Result()
		if err != nil {
			continue
		}
